...
public class GetFollowingsQueryHandler implements RequestHandler<GetFollowingsQuery, List<UserPayload>> {

    UserJpaRepository userJpaRepository;
    UserMapper mapper;

    @Override
    public List<UserPayload> handle(GetFollowingsQuery request) {
        User self = userJpaRepository.findById(request.selfId()).orElseThrow(() -> new ResourceNotFoundException("USER", "id", request.selfId()));

        Specification<User> specification = (root, query, criteriaBuilder) -> criteriaBuilder.isMember(request.userId(), root.get("followers"));
