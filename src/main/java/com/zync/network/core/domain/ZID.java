...
public final class ZID implements Serializable, Comparable<ZID> {
    @Serial
        ...
        public ZID(long mostSignificantBits, long leastSignificantBits) {
            this.msb = mostSignificantBits;
            this.lsb = leastSignificantBits;
        }

        public ZID(long time, byte[] random) {
        ...
        public static ZID fast() {
            long time = System.currentTimeMillis();
            ThreadLocalRandom random = ThreadLocalRandom.current();
            return new ZID(time << 16 | random.nextLong() & 65535L, random.nextLong());
        }

        public static ZID min(long time) {
            return new ZID(time << 16 | 0L, 0L);
        }

        public static ZID max(long time) {
            return new ZID(time << 16 | 65535L, -1L);
        }

        public static ZID from(UUID uuid) {
            return new ZID(uuid.getMostSignificantBits(), uuid.getLeastSignificantBits());
        }

        public static ZID from(byte[] bytes) {
            if (bytes != null && bytes.length == 16) {
                ...
                return new ZID(msb, lsb);
            } else {
                throw new IllegalArgumentException("Invalid zid bytes");
            }
        }

        public static ZID from(String string) {
        ...
        public boolean equals(Object other) {
            if (other == null) {
                return false;
            } else if (other.getClass() != ZID.class) {
                return false;
            } else {
                ZID that = (ZID)other;
